...
2. **Centered letterspaced text looks slightly off-center.** ✅ *Fixed 2026-07-08.* `letter-spacing` adds a trailing space to the last character; the `.enter` link compensated with `padding-left:.5em` but the `.eyebrow` (0.6em tracking) didn't, and the eyebrow wrapped awkwardly at this width ("A PORTFOLIO IN THREE / CHAPTERS"). **Fix applied:** `.eyebrow` (`index.html:136`) now carries `padding-left:.6em`, matching the same compensation convention already used by `.enter`. A new `@media (max-width:480px)` rule (`index.html:439-441`) drops it to `font-size:10px;letter-spacing:.3em;padding-left:.3em` so the phrase fits on one line instead of orphaning "CHAPTERS" alone. `h1` was left as-is — its letter-spacing is mid-animation (`track` keyframe, .3em → .06em) so a static compensation value would be imprecise, and it doesn't exhibit the wrapping problem the eyebrow does. Verified in headless Chrome (`puppeteer-core` against system Chrome) at 320/340/359/390/414/480/481/900/1440px: the eyebrow renders on a single line at every width (previously wrapped to 2 lines at ≤414px), the compensation offset is applied consistently on both sides of the breakpoint, and a 390px screenshot confirms "A PORTFOLIO IN THREE CHAPTERS" reads as one clean line above the title — zero console errors.
...
- No `404.html` (hash routing mostly avoids it, but any bad path gets the GitHub default). ✅ *Fixed 2026-07-08.* Added `404.html` at the repo root, styled to match the site (same fonts/color tokens, `theme-fall`-style forest palette, `min-height:100svh`-safe layout) with a full-bleed crop of a tiger photo, a safari-themed headline ("404 · Not Even a Pugmark" / "Our tracker checked the ground here. Nothing.") and two CTAs back to the homepage and prints wall. New image asset `images/lost-trail.webp` (135 KB, resized to 1600px long edge and re-encoded at `cwebp -q 82`, matching the site's existing image pipeline). GitHub Pages serves a root `404.html` automatically for any unmatched path on a custom domain — no extra config needed beyond the file existing. Verified in headless Chrome (`puppeteer-core` against system Chrome) at 1440×900 and 390×844: image loads (`naturalWidth`/`naturalHeight` correct, `complete: true`), zero console errors, zero failed/4xx+ requests, no horizontal overflow, and both CTA hrefs (`/`, `/#prints`) resolve correctly.
